function [E,W,x]=Spin1Jcal0_1(m,s)
% Matriu 3x3 (capsa 1: 2x2) Acoblament spin per a s=1 j=0. Autor: Sandra Tomàs (Font: Ruben Oncala)
...
% 
%disp('**************************************************')
%disp('Hybrid-Quarkonium meson states with spin coupling s=1(j=0):')
%disp('**************************************************')
%format long;
...
% aqui escollirem els parametres L i S del sistema ---------------------------------------------
...
[s]=parameters3;
  ...
  if s==1 % CAS HYBRIT QUARKONIUM P^(+,0,-)
    if j==0 
      r(1,1,i)=A(x(i),j+1)+(-2)*( -HH(x(i),j)+2*A9(x(i),j) );
      r(2,2,i)=CC(x(i),j+1)+(-2)*( +Vhf(x(i))+2*A1(x(i),j) );
     
      
      r(1,2,i)=B(x(i),j+1)+(-2)*( +A3(x(i),j)+A7(x(i),j) );
      r(2,1,i)=r(1,2,i);
