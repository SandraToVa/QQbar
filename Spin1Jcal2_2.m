function [E,W,x]=Spin1Jcal2_2(m,s)
% Matriu 9x9 (capsa 2: 4x4) Acoblament spin per a s=1 j>1. Autor: Sandra Tomàs (Font: Ruben Oncala)
% Estats np_J n(p/f)_J etc
...
%disp(' ');
%disp('Calculating eigenvalues with indices between 0 and 10:'); tol e-7
t=cputime;
tol=5e-9; %x=328
[EigvData,meshData]=computeEigenvalues(system,0,5,tol); 
 ...
 %El (-2) és el que es factor comú en la eq (4) del paper
  ...
  if s==1 % CAS HYBRIT QUARKONIUM P^(+,0,-)
    if j>=2 
      ...
      r(2,2,i)=A(x(i),j)+ (-2)*(-DD(x(i),j)+2*E9(x(i),j));
      r(3,3,i)=CC(x(i),j)+ (-2)*(+BB(x(i),j)+2*E1(x(i),j));
      r(4,4,i)=F(x(i),j+1)+ (-2)*(-BB(x(i),j)+2*A5(x(i),j));
      
      r(1,2,i)=+(-2)*(DE(x(i),j)+F8(x(i),j)+G6(x(i),j));
      r(2,1,i)=r(1,2,i);
      r(2,3,i)=B(x(i),j)+ (-2)*(+E3(x(i),j)+E7(x(i),j));
      r(3,2,i)=r(2,3,i);
      r(3,4,i)=+(-2)*(BC(x(i),j)+B4(x(i),j)+D2(x(i),j));
      r(4,3,i)=r(3,4,i);
