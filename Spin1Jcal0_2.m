function [E,W,x]=Spin1Jcal0_2(m,s)
% Matriu 3x3 (capsa 2: 1x1) Acoblament spin per a s=1 j=0. Autor: Sandra Tomàs (Font: Ruben Oncala)
...
t=cputime;
tol=5e-7; %x=101
[EigvData,meshData]=computeEigenvalues(system,0,1,tol);
...
[s]=parameters3;
  ...
  if s==1 % CAS HYBRIT QUARKONIUM P^(+,0,-)
    if j==0 
      r(1,1,i)=F(x(i),j+1)+(-2)*(-BB(x(i),j)+2*A5(x(i),j));
