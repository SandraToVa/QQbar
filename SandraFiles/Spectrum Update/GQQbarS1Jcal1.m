%Functions to compute spectrum and w.f. of hybrid spin 1 with Jcal=1
% Hybrid S=1, Jcal=1, J=0,1,2 they can be:
% - P_0-states L=1 J=0 (P_{11M}^{+-})
% - P_1-states L=1 J=1 (P_{11M}^{00})
% - (S/D)_1-states L=0,2 J=1 (P_{11M}^{+0}, P_{11M}^{-0}) wth this order
% - (P/D)_2-states L=1,3 J=2 (P_{11M}^{++}, P_{11M}^{-+})
% - D_2-states L=2 J=2 (P_{11M}^{0+})
% The order of the output wavefunction components is:
% R_{01M}, ( P_{11M}^{++}, P_{11M}^{-+}, P_{11M}^{+-}, P_{11M}^{00}, 
% P_{11M}^{0+}, P_{11M}^{+0}, P_{11M}^{-0} )
% Autor: Sandra Tomàs
% Creador subrutines i font: Ruben Oncala

function [E,W,x]=GQQbarS1Jcal1(m)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% with mixing: 
%   - with hyperfine: spectrum from Ruben very non-degenerated (n=8)
...
if mix == false
    ...
    if hf == true
        ...
        % Get diagonal and off-diagonal terms of the hyperfine splitting
            ...
            M8v44 = vMatrixM8(j, 'v44');
            M8v66 = vMatrixM8(j, 'v66');
            M8v36 = vMatrixM8(j, 'v36');
            M8v46 = vMatrixM8(j, 'v46');

            M9v22 = vMatrixM9(j, 'v22');
            M9v23 = vMatrixM9(j, 'v23');
            M9v33 = vMatrixM9(j, 'v33');
            M9v44 = vMatrixM9(j, 'v44');
            M9v66 = vMatrixM9(j, 'v66');
            M9v26 = vMatrixM9(j, 'v26');
            M9v36 = vMatrixM9(j, 'v36');
            M9v46 = vMatrixM9(j, 'v46');

            M10v11 = vMatrixM10(j, 'v11');
            M10v33 = vMatrixM10(j, 'v33');
            M10v44 = vMatrixM10(j, 'v44');
            M10v13 = vMatrixM10(j, 'v13');

            M11v11 = vMatrixM11(j, 'v11');
            M11v33 = vMatrixM11(j, 'v33');
            M11v44 = vMatrixM11(j, 'v44');
            M11v13 = vMatrixM11(j, 'v13');
            M11v14 = vMatrixM11(j, 'v14');
            M11v34 = vMatrixM11(j, 'v34');
        ...
        r(1,4,i) = - 2*Vhf2(x(i),m) * M9v26;
        r(4,1,i) = - 2*Vhf2(x(i),m) * M9v26;
    
        r(2,2,i) = v33 - 2*Vhf(x(i),m) * M8v33 - 2*Vhf2(x(i),m) * M9v33;
        r(2,4,i) = - 2*Vhf(x(i),m) * M8v36 - 2*Vhf2(x(i),m) * M9v36;
        r(4,2,i) = - 2*Vhf(x(i),m) * M8v36 - 2*Vhf2(x(i),m) * M9v36;

        r(3,3,i) = v44 - 2*Vhf(x(i),m) * M8v44 - 2*Vhf2(x(i),m) * M9v44;
        r(3,4,i) = - 2*Vhf(x(i),m) * M8v46 - 2*Vhf2(x(i),m) * M9v46;
        r(4,3,i) = - 2*Vhf(x(i),m) * M8v46 - 2*Vhf2(x(i),m) * M9v46;

        r(4,4,i) = v66 - 2*Vhf(x(i),m) * M8v66 - 2*Vhf2(x(i),m) * M9v66;

        r(5,5,i) = v11d - 2*Vhf(x(i),m) * M10v11 - 2*Vhf2(x(i),m) * M11v11;
        r(5,6,i) = - 2*Vhf(x(i),m) * M10v13 - 2*Vhf2(x(i),m) * M11v13;
        r(6,5,i) = - 2*Vhf(x(i),m) * M10v13 - 2*Vhf2(x(i),m) * M11v13;
        r(5,7,i) = - 2*Vhf2(x(i),m) * M11v14;
        r(7,5,i) = - 2*Vhf2(x(i),m) * M11v14;

        r(6,6,i) = v33d - 2*Vhf(x(i),m) * M10v33 - 2*Vhf2(x(i),m) * M11v33;
        r(6,7,i) = v34d - 2*Vhf2(x(i),m) * M11v34;
        r(7,6,i) = v34d - 2*Vhf2(x(i),m) * M11v34;
          
        r(7,7,i) = v44d - 2*Vhf(x(i),m) * M10v44 - 2*Vhf2(x(i),m) * M11v44;
...
elseif mix == true
    r = zeros(8,8,4);
        ...
        % Get diagonal and off-diagonal components of static potential
            ...
            [v12, v13, v14, ~, v16] = vOffDiagonalH1(x(i), j, m);
        ...
        r(2,5,i) = - 2*Vhf2(x(i),m) * M9v26;
        r(5,2,i) = - 2*Vhf2(x(i),m) * M9v26;
    
        r(3,3,i) = v33 - 2*Vhf(x(i),m) * M8v33 - 2*Vhf2(x(i),m) * M9v33;
        r(3,5,i) = - 2*Vhf(x(i),m) * M8v36 - 2*Vhf2(x(i),m) * M9v36;
        r(5,3,i) = - 2*Vhf(x(i),m) * M8v36 - 2*Vhf2(x(i),m) * M9v36;

        r(4,4,i) = v44 - 2*Vhf(x(i),m) * M8v44 - 2*Vhf2(x(i),m) * M9v44;
        r(4,5,i) = - 2*Vhf(x(i),m) * M8v46 - 2*Vhf2(x(i),m) * M9v46;
        r(5,4,i) = - 2*Vhf(x(i),m) * M8v46 - 2*Vhf2(x(i),m) * M9v46;

        r(5,5,i) = v66 - 2*Vhf(x(i),m) * M8v66 - 2*Vhf2(x(i),m) * M9v66;

        r(6,6,i) = v11d - 2*Vhf(x(i),m) * M10v11 - 2*Vhf2(x(i),m) * M11v11;
        r(6,7,i) = - 2*Vhf(x(i),m) * M10v13 - 2*Vhf2(x(i),m) * M11v13;
        r(7,6,i) = - 2*Vhf(x(i),m) * M10v13 - 2*Vhf2(x(i),m) * M11v13;
        r(6,8,i) = - 2*Vhf2(x(i),m) * M11v14;
        r(8,6,i) = - 2*Vhf2(x(i),m) * M11v14;

        r(7,7,i) = v33d - 2*Vhf(x(i),m) * M10v33 - 2*Vhf2(x(i),m) * M11v33;
        r(7,8,i) = v34d - 2*Vhf2(x(i),m) * M11v34;
        r(8,7,i) = v34d - 2*Vhf2(x(i),m) * M11v34;
          
        r(8,8,i) = v44d - 2*Vhf(x(i),m) * M10v44 - 2*Vhf2(x(i),m) * M11v44;
        end
