%Functions to compute spectrum and w.f. of hybrids with J=1
% Hybrid S=0, Jcal=1, J=1 hey can be several states:
% - (S/D)_1-States L=0,L=2
% - P_1-states L=1
% Autor: Sandra Tomàs
% Creador subrutines i font: Ruben Oncala

function [E,W,x]=GQQbarS0Jcal1(m)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%---------Coose n!----------
% without mixing just hyrbid spectrum n=3
...
function val = vHybrid(x, j, m, type)
    switch type
        %v11 and v22 are flipped with reference to Rubens notes and paper
        %to agree with the mixing order of (s/d)1 so first row = d, and
        %second = s
        case 'v22', val = ((j-1)*j)./(x.^2) + m.*VSigU(x,m) + m.*Vq(x,m).*(j+1)/(2*j + 1);
        case 'v11', val = ((j+1)*(j+2))./(x.^2) + m.*VSigU(x,m) + m.*Vq(x,m).*(j)/(2*j+1);
        case 'v33', val = (j*(j+1))./(x.^2) + m.*VPiU(x,m);
        case 'v12', val = m.*Vq(x,m).*sqrt(j*(j+1)/(2*j+1));
    end
end
